            ...
            {items.styles.map((item, index) => (
              <div
                ...
                className={classnames(
                  "hover:bg-gray-200 hover:cursor-pointer grid-rows-6 rounded-xl py-1",
                  { "bg-gray-200": style === index }
                )}
              >
                <img alt="styleImage" src={item.imageSource} />
                <div className="flex justify-center text-gray-800 font-semibold">
                  {item.name}
                </div>
              </div>
            ))}
          ...
          <div>
            <div>
                ...
                {items.size.map((item, index) =>
                  item.type === "*" ? (
                    <div
                      key={index}
                      onClick={() => {
                        setSelectedSizeState(item.value);
                      }}
                      className={classnames(
                        "hover:bg-gray-600 hover:cursor-pointer rounded-md p-3 text-white font-semibold grid-rows-6 flex justify-start items-center",
                        {
                          "bg-gray-600": selectedSizeState === item.value,
                          "bg-gray-400": selectedSizeState !== item.value,
                        }
                      )}
                    >
              ...
              <div className="my-2">
                <RadioButton
                  items={SidingData}
                  setValue={setSidingState}
                  defaultValue={"smartside"}
                />
              </div>
              ...
              {items.type === "Shed Series" && (
                <>
                  ...
                  <div className="my-2">
                    <RadioButton
                      items={SidewallData}
                      setValue={setSidewallState}
                      defaultValue={"Sidewall-6"}
                    />
                  </div>
                </>
              )}
